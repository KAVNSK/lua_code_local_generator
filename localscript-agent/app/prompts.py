...
def repair_user_message_structured(
    *,
    task_prompt: str,
    broken_code: str,
    context: dict | None,
    ...
    repeated_code: bool = False,
) -> str:
    parts = [
        "You are repairing Lua that failed deterministic validation.",
        "Return ONLY corrected Lua code.",
        f"Attempt: {attempt_idx}",
        f"Task reminder:\n{task_prompt.strip()}",
    ]
    if context is not None:
        parts.append(
            "Context (read-only wf data):\n" + json.dumps(context, ensure_ascii=False, indent=2)
        )

    if repeated_code:
        parts.append(
            "Important: previous candidate repeated an already-seen failing version. "
            "Use a different strategy and do NOT output equivalent code."
        )
    ...
    if prior_failures:
        parts.append(
            ...
            + json.dumps(prior_failures[-4:], ensure_ascii=False, indent=2)
        )
